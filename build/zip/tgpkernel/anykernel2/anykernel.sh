# -------------------------------
# TGPKERNEL AROMA INSTALLER 4.5.0
# anykernel2 portion
#
...
# Ramdisk changes - Note FE
if egrep -q "install=1" "/tmp/aroma/check_nfe.prop"; then
	ui_print "- Patching for Note FE ROMs"
	echo $NFEOSLEVEL > $split_img/boot.img-oslevel
	echo $NFEOSVERSION > $split_img/boot.img-osversion
	cp -rf $patch/ramdisk-nfe/* $ramdisk
	chmod 755 $ramdisk/init
	chmod 644 $ramdisk/init.environ.rc
...
if egrep -q "install=1" "/tmp/aroma/check_s8.prop"; then
	...
	echo $S8OSVERSION > $split_img/boot.img-osversion
...
if egrep -q "install=1" "/tmp/aroma/insecureadb.prop"; then
	...
	if egrep -q "install=1" "/tmp/aroma/check_s7.prop"; then
	...
	if egrep -q "install=1" "/tmp/aroma/check_s8.prop"; then
		cp -rf $patch/adbd-s8/* $ramdisk/sbin
		chmod 755 $ramdisk/sbin/adbd
	fi
	if egrep -q "install=1" "/tmp/aroma/check_n8.prop"; then
		cp -rf $patch/adbd-n8/* $ramdisk/sbin
		chmod 755 $ramdisk/sbin/adbd
	fi
fi

...
if [ $OPTION == "setup" ]; then
	...
	if grep -q G935F /tmp/variant_model; then
...
if [ $OPTION == "rom_check" ]; then
	...
	# Set for NFE
	if grep -q grace $BUILDPROP; then
		echo "install=0" > $AROMA/check_s7.prop
		echo "install=0" > $AROMA/check_s8.prop
		echo "install=1" > $AROMA/check_nfe.prop
	fi
...
if [ $OPTION == "config_backup" ]; then
	...
	[ -f "$CONFIG/check_nfe.prop" ] && rm -f $CONFIG/check_nfe.prop
